 ...
 * cobyla.c -
 *
 * C implementation of Mike Powell's COBYLA algorithm for minimizing a function of a few
...
/* To re-use as much as the code for the reverse communication routines, we use a trick
   which consists in "self-including" this file with different macros (COBYLA_PART1_,
   COBYLA_PART2_, etc.) defined so as to skip or modify certain parts of the source
   file. */
...
static void
trstlp(const INTEGER n, const INTEGER m, const REAL a[], const REAL b[], const REAL rho,
       REAL dx[], INTEGER* ifull, INTEGER iact[], REAL z[], REAL zdota[], REAL vmultc[],
       REAL sdirn[], REAL dxnew[], REAL vmultd[]);

static void
print_calcfc(FILE* output, INTEGER n, INTEGER nfvals, REAL f, REAL maxcv, const REAL x[]);
...
/* CALCFC_WRAPPER is a wrapper function, it assumes a CALCFC subroutine written in
   FORTRAN. */
static REAL
...
int
FORTRAN_NAME(cobyla,COBYLA)(INTEGER* n, INTEGER* m, REAL x[], REAL* rhobeg, REAL* rhoend,
...
/* Include this file with the macro COBYLA_REVCOM_ defined to generate the code
   `newuoa_iterate` in place of `newuob`. */
...
#endif
{
    ...
    /* Set the initial values of some parameters. The last column of SIM holds the optimal
       vertex of the current simplex, and the preceding N columns hold the displacements
       from the optimal vertex to the other vertices. Further, SIMI holds the inverse of
       the matrix that is contained in the first N columns of SIM. */
    np = n + 1;
    mp = m + 1;
    mpp = m + 2;
#ifdef COBYLA_REVCOM_
    if (nfvals == 0) {
        ...
        /* This is not the first function evaluation. Restore other local variables and
           jump to the place where function value is expected. */
        RESTORE(parmu);
        RESTORE(parsig);
        RESTORE(prerec);
        RESTORE(prerem);
    ...
    /* Make the next call of the user-supplied subroutine CALCFC. These instructions are
       also used for calling CALCFC during the iterations of the algorithm. */
 call_calcfc:
    ...
    if (status == COBYLA_INITIAL_ITERATE) {
        /* We already know the function value. */
        status = COBYLA_ITERATE;
    } else {
        /* Request a new function evaluation from the caller and branch to save local
           variables in the context structure. */
        status = COBYLA_ITERATE;
        goto save;
    }
 ...
 new_eval:
    ...
    /* Set the recently calculated function values in a column of DATMAT. This array has a
       column for each vertex of the current simplex, the entries of each column being the
       values of the constraint functions (if any) followed by the objective function and
       the greatest constraint violation at the vertex. */
    LOOP(k,mpp) {
    ...
    /* Exchange the new vertex of the initial simplex with the optimal vertex if
       necessary. Then, if the initial simplex is not complete, pick its next vertex and
       calculate the function values there. */
    if (jdrop <= n) {
        if (DATMAT(mp,np) <= f) {
 ...
 L_140:
    ...
    /* Switch the best vertex into pole position if it is not there already, and also
       update SIM, SIMI and DATMAT. */
    if (nbest <= n) {
        LOOP(i,mpp) {
            temp = DATMAT(i,np);
    ...
    /* Make an error return if SIGI is a poor approximation to the inverse of the leading
       N by N submatrix of SIG. */
    error = zero;
    LOOP(i,n) {
        LOOP(j,n) {
            temp = (i == j ? -one : zero);
    ...
    /* Calculate the coefficients of the linear approximations to the objective and
       constraint functions, placing minus the objective function gradient after the
       constraint gradients in the array A. The vector W is used for working space. */
    LOOP(k,mp) {
        CON(k) = -DATMAT(k,np);
        LOOP(j,n) {
    ...
    /* Calculate the values of SIGMA and ETA, and set IFLAG=0 if the current simplex is
       not acceptable. */
    iflag = 1;
    parsig = alpha*rho;
    pareta = beta*rho;
    ...
    /* If a new vertex is needed to improve acceptability, then decide which vertex to
       drop from the simplex. */
 ...
 L_370:
    ...
    /* Predict the change to F and the new maximum constraint violation if the variables
       are altered from X(0) to X(0)+DX. */
    resnew = zero;
    CON(mp) = zero;
    sum = zero; /* FIXME: `sum` was uninitialized */
    LOOP(k,mp) {
    ...
    /* Increase PARMU if necessary and branch back if this change alters the optimal
       vertex. Otherwise PREREM and PREREC will be set to the predicted reductions in the
       merit function and the maximum constraint violation respectively. */
    ...
    /* Calculate the constraint and objective functions at X(*). Then find the actual
 ...
 L_440:
    ...
    /* Begin the operations that decide whether X(*) should replace one of the vertices of
       the current simplex, the change being mandatory if TRURED is positive. Firstly,
       JDROP is set to the index of the vertex that is to be replaced. */
 ...
 save:
    ...
    /* Save value of objective function, of worst constraint and of number of function
       evaluations and return status. */
    work[0] = (maximize ? -f : f);
    work[1] = resmax;
    iact[0] = nfvals;
    return status;
...
/*
 * This subroutine calculates an N-component vector DX by applying the following two
 * stages. In the first stage, DX is set to the shortest vector that minimizes the
 * greatest violation of the constraints:
 *
 *     A(1,K)*DX(1)+A(2,K)*DX(2)+...+A(N,K)*DX(N)  >=  B(K), K=2,3,...,M,
 *
 * subject to the Euclidean length of DX being at most RHO. If its length is strictly less
 * than RHO, then we use the resultant freedom in DX to minimize the objective function:
 *
 *     -A(1,M+1)*DX(1)-A(2,M+1)*DX(2)-...-A(N,M+1)*DX(N)
 *
 * subject to no increase in any greatest constraint violation. This notation allows the
 * gradient of the objective function to be regarded as the gradient of a constraint.
 * Therefore the two stages are distinguished by MCON = M and MCON > M respectively. It is
 * possible that a degeneracy may prevent DX from attaining the target length RHO. Then
 * the value IFULL=0 would be set, but usually IFULL=1 on return.
 *
 * In general NACT is the number of constraints in the active set and
 * IACT(1),...,IACT(NACT) are their indices, while the remainder of IACT contains a
 * permutation of the remaining constraint indices. Further, Z is an orthogonal matrix
 * whose first NACT columns can be regarded as the result of Gram-Schmidt applied to the
 * active constraint gradients. For J=1,2,...,NACT, the number ZDOTA(J) is the scalar
 * product of the J-th column of Z with the gradient of the J-th active constraint. DX is
 * the current vector of variables and here the residuals of the active constraints should
 * be zero. Further, the active constraints have nonnegative Lagrange multipliers that are
 * held at the beginning of VMULTC. The remainder of this vector holds the residuals of
 * the inactive constraints at DX, the ordering of the components of VMULTC being in
 * agreement with the permutation of the indices of the constraints that is in IACT. All
 * these residuals are nonnegative, which is achieved by the shift RESMAX that makes the
 * least residual zero.
 *
 * Initialize Z and some other variables. The value of RESMAX will be appropriate to DX=0,
 * while ICON will be the index of a most violated constraint if RESMAX is positive.
 * Usually during the first stage the vector SDIRN gives a search direction that reduces
 * all the active constraint violations by one simultaneously.
 */
       ...
       REAL zdota[], REAL vmultc[], REAL sdirn[], REAL dxnew[], REAL vmultd[])
{
    ...
    REAL acca, accb, alpha, beta, dd, optnew, optold, ratio, resmax, resold, sd, sp,
        spabs, ss, step, stpful, sum, sumabs, temp, tempa, tempb, tot, vsave, zdotv,
        zdotw, zdvabs, zdwabs;
    ...
    *ifull = 1;
    ...
    /* End the current stage of the calculation if 3 consecutive iterations have either
       failed to reduce the best calculated value of the objective function or to increase
       the number of active constraints since the best value was calculated. This strategy
       prevents cycling, but there is a remote possibility that it will cause premature
       termination. */
 L_60:
    optold = zero;
    icount = 0;
 L_70:
    ...
    /* If ICON exceeds NACT, then we add the constraint with index IACT(ICON) to the
       active set. Apply Givens rotations so that the last N-NACT-1 columns of Z are
       orthogonal to the gradient of the new constraint, a scalar product being set to
       zero if its nonzero value could be due to computer rounding errors. The array DXNEW
    ...
    /* Add the new constraint if this can be done without a deletion from the active
       set. */
    if (tot != zero) {
        ++nact;
        ZDOTA(nact) = tot;
    ...
    /* The next instruction is reached if a deletion has to be made from the active set in
       order to make room for the new active constraint, because the new constraint
       gradient is a linear combination of the gradients of the old active constraints.
       Set the elements of VMULTD to the multipliers of the linear combination. Further,
       set IOUT to the index of the constraint to be deleted, but branch if no suitable
       index can be found. */
    ratio = -one;
    k = nact;
 L_130:
    ...
    /* Revise the Lagrange multipliers and reorder the active constraints so that the one
       to be replaced is at the end of the list. Also calculate the new value of
       ZDOTA(NACT) and branch if it is not acceptable. */
    LOOP(k,nact) {
    ...
    /* Update IACT and ensure that the objective function continues to be treated as the
       last active constraint when MCON > M. */
 L_210:
    IACT(icon) = IACT(nact);
    IACT(nact) = kk;
    if (mcon > m && kk != mcon) {
    ...
    /* If stage one is in progress, then set SDIRN to the direction of the next change to
 ...
 L_320:
    ...
    /* Calculate the step to the boundary of the trust region or take the step that
       reduces RESMAX to zero. The two statements below that include the factor 1.0E-6
       prevent some harmless underflows that occurred in a test calculation. Further, we
       skip the step if it could be zero within a reasonable tolerance for computer
       rounding errors. */
 L_340:
    dd = rho*rho;
    sd = zero;
    ...
    /* Set DXNEW to the new variables if STEP is the steplength, and reduce RESMAX to the
       corresponding maximum residual if stage one is being done. Because DXNEW will be
       changed during the calculation of some Lagrange multipliers, it will be restored to
       the following value later. */
    LOOP(i,n) {
    ...
    /* Set VMULTD to the VMULTC vector that would occur if DX became DXNEW. A device is
       included to force VMULTD(K) = 0 if deviations from this value can be attributed to
       computer rounding errors. First calculate the new Lagrange multipliers. */
    k = nact;
    for (;;) {
        zdotw = zero;
        zdwabs = zero;
    ...
    /* If the full step is not acceptable then begin another iteration. Otherwise switch
       to stage two or end the calculation. */
    if (icon > 0) goto L_70;
 ...
 L_480:
    ...
    /* We employ any freedom that may be available to reduce the objective function before
       returning a DX whose length is less than RHO. */
 L_490:
