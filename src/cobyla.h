 ...
 * cobyla.h -
 *
 * Definitions for Mike Powell's COBYLA algorithm for minimizing a function of a few
 ...
 *
 * Prototype of the objective function assumed by the COBYLA routine. The returned value
 * is the function value at `x`, `n` is the number of variables, `m` is the number of
 ...
 *
 * This type enumerate the possible values returned by cobyla(), cobyla_get_status() and
 * cobyla_iterate().
 */
typedef enum {
 ...
 *
 * The `cobyla` algorithm minimizes an objective function `f(x)` subject to `m` inequality
 * constraints on `x`, where `x` is a vector of variables that has `n` components. The
 * algorithm employs linear approximations to the objective and constraint functions, the
 ...
 * `rhoend`. For each `rho` the subroutine tries to achieve a good vector of variables for
 * the current size, and then `rho` is reduced until the value `rhoend` is reached.
 * Therefore `rhobeg` and `rhoend` should be set to reasonable initial changes to and the
 * required accuracy in the variables respectively, but this accuracy should be viewed as
 * a subject for experimentation because it is not guaranteed. The subroutine has an
 * advantage over many of its competitors, however, which is that it treats each
 * constraint individually when calculating a change to the variables, instead of lumping
 * the constraints together into a single penalty function. The name of the subroutine is
 ...
 * initial vector of variables in `x`. Further, the value of `iprint` should be set to 0,
 * 1, 2 or 3, which controls the amount of printing during the calculation. Specifically,
 * there is no output if `iprint=0` and there is output only at the end of the calculation
 * if `iprint=1`. Otherwise each new value of `rho` and `sigma` is printed. Further, the
 * vector of variables and some function information are given either when `rho` is
 * reduced or when each new value of `f(x)` is computed in the cases `iprint=2` or
 * `iprint=3` respectively. Here `sigma` is a penalty parameter, it being assumed that a
 * change to `x` is an improvement if it reduces the merit function:
 *
 *     f(x) + sigma*max(0.0, -C1(x), -C2(x), ..., -CM(x)),
 *
 * where `C1`, `C2`, ..., `CM` denote the constraint functions that should become
 * nonnegative eventually, at least to the precision of `rhoend`. In the printed output
 * the displayed term that is multiplied by `sigma` is called `maxcv`, which stands for
 * "MAXimum Constraint Violation". The argument `maxfun` is the address of an integer
 * variable that must be set by the user to a limit on the number of calls of `fc`, the
 * purpose of this routine being given below. The value of `maxfun` will be altered to the
 * number of calls of `fc` that are made. The arguments `work` and `iact` provide real and
 * integer arrays that are used as working space. Their lengths must be at least
 * `n*(3*n+2*m+11)+4*m+6` and `m+1` respectively.
 *
 * In order to define the objective and constraint functions, we require a function `fc`
 * that has the following prototype:
 *
 *     REAL fc(INTEGER n, INTEGER m, const REAL x[], REAL con[], void* data);
 *
 * The values of `n` and `m` are fixed and have been defined already, while `x` is now the
 * current vector of variables. The function should return the value of the objective
 * function at `x` and store constraint functions at `x` in `con[0]`, `con[1]`, ...,
 * `con[m-1]`. Argument `data` will be set with whatever value has been provided when
 * `cobyla` was called. Note that we are trying to adjust `x` so that `f(x)` is as small
 * as possible subject to the constraint functions being nonnegative.
 *
 ...
 *
 * This function is a variant of `cobyla` which attempts to minimize or maximize an
 * objective function of many variables subject to inequality constraints. The scaling of
 * the variables is important for the success of the algorithm and the `scale` argument
 * (if not `NULL`) should specify the relative size of each variable. If specified,
 * `scale` is an array of `n` strictly nonnegative values, such that `scale[i]*rho` (with
 * `rho` the trust region radius) is the size of the trust region for the `i`-th variable.
 * Thus `scale[i]*rhobeg` is the typical step size for the `i`-th variable at the
 * beginning of the algorithm and `scale[i]*rhoend` is the typical precision on the `i`-th
 * variable at the end. If `scale` is not specified, a unit scaling for all the variables
 * is assumed.
 *
 * @param n      - The number of variables.
 *
 * @param m      - The number of constraints.
 *
 * @param maximize - Non-zero to attempt to maximize the objective function; zero to
 ...
 *
 * @param scale  - An array of `n` scaling factors, all strictly positive. Can be `NULL`
 *                 to perform no scaling of the variables.
 *
 * @param rhobeg - The initial trust region radius.
 *
 * @param rhoend - The final trust region radius.
 ...
 *
 * @return `COBYLA_SUCCESS` is returned when the algorithm is successful; any other value
...
/* Allocate a new reverse communication workspace for COBYLA algorithm. The returned
   address is `NULL` to indicate an error due to either invalid parameters (external
   variable `errno` set to `EINVAL`), or insufficient memory (external variable `errno`
   set to `ENOMEM`). When no longer needed, the workspace must be deleted with
   `cobyla_delete`.
...
/* Perform the next iteration of the reverse communication variant of the COBYLA
   ...
   function value and the constraints at `x`. On exit, the returned value (the new
   workspace status) is: `COBYLA_ITERATE` if a new trial point has been stored in `x` and
   if user is requested to compute the function value and the constraints at the new
   point; `COBYLA_SUCCESS` if algorithm has converged and `x` has been set with the
   variables at the solution (the corresponding function value can be retrieved with
   `cobyla_get_last_f`); anything else indicates an error (see `cobyla_reason` for an
   explanatory message). */
extern cobyla_status cobyla_iterate(
    cobyla_context* ctx, double f, double x[], double c[]);

/* Restart COBYLA algorithm using the same parameters. The return value is the new status
   of the algorithm, see `cobyla_get_status` for details. */
extern cobyla_status cobyla_restart(cobyla_context* ctx);

/* Get the current status of the algorithm. Result is: `COBYLA_ITERATE` if user is
   requested to compute F(X) and C(X); `COBYLA_SUCCESS` if algorithm has converged;
   anything else indicates an error (see `cobyla_reason` for an explanatory message). */
...
/*
 * Subroutine variant designed to be callable from FORTRAN code. Usage is similar to that
 * of `cobyla` except that everything is passed by address and that, in order to define
 * the objective and constraint functions, we require a subroutine that has the name and
 * arguments:
 *
 ...
 *
 * The values of N and M are fixed and have been defined already, while X is now the
 * current vector of variables. The subroutine should return the objective and constraint
 * functions at X in F and CON(1),CON(2), ...,CON(M). Note that we are trying to adjust X
